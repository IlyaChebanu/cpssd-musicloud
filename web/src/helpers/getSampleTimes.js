export default (audioContext, state, sample) => {
  ...
  if (state.loopEnabled && sample.time < currentBeat && currentBeat > state.loop.stop - 0.1 * beatsPerSecond) {
  ...
  const endTime = audioContext.currentTime + sample.duration + (sampleTime - currentBeat) * secondsPerBeat;
